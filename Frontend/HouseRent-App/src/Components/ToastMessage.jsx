...
function ToastMessage({ message, type }) {
  ...
  return (
    <div
      className={`flex items-center w-full max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow fixed top-4 right-4 z-50`} // Positioning at top right
      role="alert"
    >
      <div
        className={`inline-flex items-center justify-center flex-shrink-0 w-8 h-8 ${toastStyles[type]}`}
      >
        <svg
          className="w-5 h-5"
          aria-hidden="true"
        ...
        <span className="sr-only">
          {type === "success" ? "Check" : "Warning"} icon
        </span>
      </div>
